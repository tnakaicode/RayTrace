...
from pvtrace.light.light import cube_mask, rectangular_mask
...
from pvtrace import Node, Sphere, Material, Scene, MeshcatRenderer, Ray, photon_tracer, lumogen_f_red_305, Luminophore, isotropic, Absorber, Light, cone, Distribution
...
# Square emitter
light = Light(position=functools.partial(rectangular_mask, 1, 0.3))
vis = MeshcatRenderer()
vis.render(scene)
for ray in light.emit(100):
    steps = photon_tracer.follow(scene, ray)
...
plt.hist([x.wavelength for x in list(light.emit(10000))],
         bins=20, density=True, histtype='step', label='sample')
