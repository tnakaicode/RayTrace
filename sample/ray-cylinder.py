...
def ray_z_cylinder(length, radius, ray_origin, ray_direction):
    ...
    tcyl = [t for t in np.roots([a, b, c]) if np.isfinite(
        t) and np.isreal(t) and t >= 0]
    ...
    cyl_candidates = [(point, t) for (
        point, t) in cyl_candidates if point[2] > -0.5 * length and point[2] < 0.5 * length]
    ...
    points = tuple([tuple(p.tolist())
                    for p in list(zip(*intersection_info))[0]])
    return points
