...
def ABC(x):
    ...
    yy = x[1]
    zz = x[2]
    BB[0] = np.cos(kk * yy) + np.sin(kk * zz)
    dBB[0, 0] = 0
    dBB[0, 1] = -np.sin(kk * yy)
    dBB[0, 2] = np.cos(kk * zz)
    BB[1] = np.sin(kk * xx) + np.cos(kk * zz)
    dBB[1, 0] = np.cos(kk * xx)
    dBB[1, 1] = 0
    dBB[1, 2] = -np.sin(kk * zz)
    BB[2] = np.sin(kk * yy) + np.cos(kk * xx)
    dBB[2, 0] = -np.sin(kk * xx)
    dBB[2, 1] = np.cos(kk * yy)
    dBB[2, 2] = 0
    return BB, dBB * kk
...
def diagnostic(y, t, counter, fname, ldiag2file):
    ...
    if ldiag2file == 1:
        fname.write(str(t) + "\t" + str(x[0]) + "\t" + str(x[1]) + "\t" + str(
            x[2]) + "\t" + str(rsqr) + "\t" + str(mFTLE) + "\n")
